...
class projection_MLP(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim=2048):
        super().__init__()
        ...
        self.layer1 = nn.Sequential(
            ...
            nn.BatchNorm1d(hidden_dim),#bath内的全部数据的均值然后标准化
...
class attention(nn.Module):
    def __init__(self,dim_in, dim_k,dim_v):
        ...
        self.dim_v = dim_v
        self.args = get_args()
        self.linear_q = nn.Sequential(  # f encoder
            nn.Linear(dim_in, dim_k),
            nn.BatchNorm1d(dim_k)
        )
        self.linear_k = nn.Sequential(  # f encoder
            nn.Linear(dim_in, dim_k),
            nn.BatchNorm1d(dim_k)
        )
        self.linear_v = nn.Sequential(  # f encoder
            nn.Linear(dim_in, dim_v),
    ...
    def forward(self,x,mask):
        ...
        q,k,v= q.view(self.args.batch_size,self.args.max_len,self.args.emb_dim),k.view(self.args.batch_size,self.args.max_len,self.args.emb_dim),v.view(self.args.batch_size,self.args.max_len,self.args.emb_dim)
...
class Plus_Proj_layer(nn.Module):#   继承类nn.Module
    def __init__(self, backbone):
        ...
        self.attention_data=attention(self.args.max_len*self.args.emb_dim, self.args.max_len*self.args.emb_dim, self.args.max_len*self.args.emb_dim)
        ...
        if self.args.backbone=='textcnn':
            self.projector = projection_MLP(model_opt.model_dim,2048)
    ...
    def forward(self, x1, x2, mask):
        x1=self.attention_data(x1.view(self.args.batch_size,self.args.max_len*self.args.emb_dim),mask)
        x2=self.attention_data(x2.view(self.args.batch_size,self.args.max_len*self.args.emb_dim),mask)
