...
class ModelConfig(object):
    ...
    kernel_sizes = [1,1,1,6,10,15] # 不同尺寸的kernel#,3,4,5,6数据太小（小余一万）不适合用大的参数#1,1,1,1达到47.78#6 15 20
    dropout = 0.5 # dropout
...
class ModelCNN(nn.Module):
    ...
    def __init__(self,  kernel_num, kernel_sizes, model_dim):
        ...
        # 全连接层
        #self.fc = nn.Linear(len(kernel_sizes) * kernel_num, model_dim)
    ...
    def forward(self, x):
        ...
        # dropout
        #x = self.dropout(x)
        # fc#  在cnn特征后面不能跟任何层  不然在cl训练中会大大的影响cnn所学习到的特征
